      ...
      <div className="w-full block m-auto px-5 pt-0 md:pb-[64px] max-w-[1296px] -translate-y-10">
        <div className="flex gap-3 w-full items-stretch flex-col md:flex-row mx-auto flex-wrap mb-10 md:mb-16">
          {triviaData.triviaSection.map((trivia, index) => (
            <div key={index} className="w-full flex flex-col p-4 md:p-6 bg-white rounded items-stretch flex-1 min-w-[270px]">
              ...
              <p className={`font-krona text-primary text-2xl text-start font-medium`}>{trivia.number}</p>
        ...
        <div className="w-full z-10 relative">
          ...
          <div className="flex gap-3 w-full flex-wrap">
            {triviaData.recommendationSection.map((rec, index) => (
              <div key={index} className="w-full p-4 md:p-6 bg-white flex-1 min-w-[280px] relative rounded">
                <ArrowUpRight width={20} height={20} className="absolute right-4 top-4 text-secondary" />
                <p className="text-secondary text-base font-medium">{rec.recommendedBy}</p>
                <a className="text-black underline hover:text-blue-600 transition-all cursor-pointer text-sm" href={rec.url}>{rec.recommendation}</a>
              </div>
            ))}
          </div>
        </div>
